...
:CreateSourceCode
	...
	SET FS=%2
	SET BDF=bdf/%FN%-%FS%.bdf
	echo Try to generate source code code/u8g2_%FN%_%FS%_fontUniSensorChinese.c
	bdfconv -b 0 -f 1 -M map/fontUniSensorChinese.map -n u8g2_%FN%_%FS%_fontUniSensorChinese -o code/u8g2_%FN%_%FS%_fontUniSensorChinese.c %BDF% -d bdf/simsun-9.bdf
GOTO:EOF

...
/**************************************************************************/
/*!
	@brief 用于生成构建时间的内存友好构造函数。

    此版本旨在通过保留日期和时间来节省 RAM
    程序存储器中的字符串。将它与“F()”宏一起使用：

    ```
    日期时间 buildTime(F(__DATE__), F(__TIME__));
    ```

    @param date 日期 PROGMEM 字符串，例如F（“20​​20 年 4 月 16 日”）。
    @param time Time PROGMEM 字符串，例如F（“18:34:56”）。
    ************************************************************************
...
DateTime::DateTime(const char *iso8601dateTime) {
  ...
  memcpy(ref, iso8601dateTime, Ardmin(strlen(ref), strlen(iso8601dateTime)));
  yOff = conv2d(ref + 2);
  m = conv2d(ref + 5);
  d = conv2d(ref + 8);
  hh = conv2d(ref + 11);
...
*/
/**************************************************************************/
char* DateTime::timestamp(timestampOpt opt) const {
  static char buffer[25]; // large enough for any DateTime, including invalid ones
